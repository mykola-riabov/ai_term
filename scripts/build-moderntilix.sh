#!/usr/bin/env bash
# Build Modern Tilix (system apt deps if available, else local sysroot).
set -euo pipefail
...
if command -v ldc2 >/dev/null && pkg-config --exists gtkd-3 2>/dev/null; then
  ...
  BUILD_ROOT="${MODERN_TILIX_BUILD:-$HOME/.local/moderntilix-build}"
...
ninja -C build moderntilix
echo "OK: $ROOT/build/moderntilix"
