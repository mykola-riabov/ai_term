...
import gx.aiterm.customtitle;
import gx.aiterm.prefeditor.titleeditor;
import gx.aiterm.preferences;
import gx.aiterm.session;
import gx.aiterm.sidebar;
import gx.aiterm.modern.quickbar;
import gx.aiterm.modern.store;

/**
 * The GTK Application Window for Aiterm. It is responsible for
...
private:
    ...
    void createUI() {
        ...
        if (useTabs) {
            nb.getStyleContext().addClass("aiterm-background");
            nb.setScrollable(true);
            nb.setGroupName("aiterm");
        ...
        nb.addOnSwitchPage(delegate(Widget page, uint, Notebook) {
            ...
            aiterm.withdrawNotification(session.uuid);
        }, ConnectFlags.AFTER);
        if (!useTabs) {
            sb = new SideBar();
        ...
        if (isQuake() || isCSDDisabled()) {
            ...
            box.add(hb);
            modernBar = new ModernQuickBar(&feedActiveTerminal);
            addModernDataListener(&refreshModernBar);
            box.add(modernBar);
        ...
        this.addOnDestroy(delegate(Widget) {
            removeModernDataListener(&refreshModernBar);
        });
    }

public:

    void refreshModernBar() {
        if (modernBar !is null) modernBar.rebuild();
    }

    void feedActiveTerminal(string text) {
        Session session = getCurrentSession();
        if (session is null) return;
        ITerminal term = session.getActiveTerminal();
        if (term !is null) term.injectCommand(text);
    }
...
private:
    ...
    HeaderBar createHeaderBar() {
        ...
        Button btnAddHorizontal = new Button("aiterm-add-horizontal-symbolic", IconSize.MENU);
        ...
        Button btnAddVertical = new Button("aiterm-add-vertical-symbolic", IconSize.MENU);
     ...
     * https://github.com/gnunn1/aiterm/issues/342
     */
    void createDelegatedTerminalActions(GSettings gsShortcuts) {
        import gx.aiterm.terminal.terminal : Terminal;
...
//        mFileSection.appendItem(new GMenuItem(_("Close Session"), getActionDetailedName(ACTION_PREFIX, ACTION_SESSION_CLOSE)));
        ...
        mPrefSection.append(_("About Aiterm"), getActionDetailedName(ACTION_PREFIX_APP, ACTION_ABOUT));
    ...
    void onSessionAttach(string sessionUUID) {
        ...
        session = cast(Session) aiterm.findWidgetForUUID(sessionUUID);
        if (session is null) {
    ...
    AppWindow cloneWindow() {
        AppWindow result = new AppWindow(aiterm, useTabs);
        aiterm.addAppWindow(result);
    ...
    void onSessionDetach(Session session, int x, int y, bool isNewSession) {
        ...
        AppWindow window = cloneWindow();//new AppWindow(aiterm);
        aiterm.addAppWindow(window);
    ...
    void onWindowShow(Widget) {
        if (aiterm.getGlobalOverrides().maximize) {
            maximize();
        } else if (aiterm.getGlobalOverrides().minimize) {
            iconify();
    ...
    void setWindowStyle() {
        ...
        if (aiterm.getGlobalOverrides().windowStyle.length > 0) {
            foreach(i, style; SETTINGS_WINDOW_STYLE_VALUES) {
                if (style == aiterm.getGlobalOverrides().windowStyle) {
                    windowStyle = i;
                    break;
                }
            }
...
public:
    ...
    void initialize() {
        if (aiterm.getGlobalOverrides().session.length > 0) {
            foreach (sessionFilename; aiterm.getGlobalOverrides().session) {
                try {
                    if (!exists(sessionFilename)) {
                        string filename = buildPath(aiterm.getGlobalOverrides().cwd, sessionFilename);
    ...
    void createSession() {
        ...
        if (aiterm.getGlobalOverrides().cwd.length ==0 && aiterm.getGlobalOverrides().workingDir.length == 0) {
            ITerminal terminal = getActiveTerminal();
            if (terminal !is null) {
    ...
    ImageSurface getBackgroundImage(Widget widget) {
        ...
        ImageSurface surface = aiterm.getBackgroundImage();
        if (surface is null) {
            if (isBGImage !is null) {
                isBGImage.destroy();
                isBGImage = null;
...
public:
	...
	this(PositionType position, string text, Session session) {
        ...
        evNotifications.getStyleContext().addClass("aiterm-notification-count");
        ...
        button.getStyleContext().addClass("aiterm-small-button");
