...
import gx.aiterm.terminal.terminal;
...
private:
    ...
    void createBaseUI() {
        ...
        stackGroup.getStyleContext().addClass("aiterm-background");
        addNamed(stackGroup, STACK_GROUP_NAME);
        stackMaximized = new Box(Orientation.VERTICAL, 0);
        stackMaximized.getStyleContext().addClass("aiterm-background");
    ...
    void onTerminalRequestMove(string srcUUID, Terminal dest, DragQuadrant dq) {
        ...
        if (src !is null) {
            ...
            src = cast(Terminal) aiterm.findWidgetForUUID(srcUUID);
            if (src is null) {
...
private:
    ...
    void initSession() {
        ...
        getStyleContext.addClass("aiterm-background");
...
public:
    ...
    void withdrawNotification() {
        ...
        foreach (terminal; terminals) {
            aiterm.withdrawNotification(terminal.uuid);
        }
    }
...
public:
     ...
     * ratio, so pre-aiterm 1.4.0 behavior. See issue #613
     */
...
private:
    ...
    void updateResizeProperty(PanedNode node) {
        ...
        //Thanks to tip from egmontkob, see issue https://github.com/gnunn1/aiterm/issues/161
