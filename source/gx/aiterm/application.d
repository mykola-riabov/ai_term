...
/**
 * The GTK Application used by Aiterm.
 */
class Aiterm : Application {
...
private:
    ...
    void loadResources() {
        ...
        if (findResource(APPLICATION_RESOURCES, true)) {
            ...
            //Check if aiterm has a theme specific CSS file to load
            string theme = getGtkTheme();
            string cssURI = APPLICATION_RESOURCE_ROOT ~ "/css/aiterm." ~ theme ~ ".css";
    ...
    void setupPrimaryMenuActions() {
        ...
        registerAction(this, ACTION_PREFIX_APP, ACTION_QUIT, null, delegate(GVariant, SimpleAction) { quitAiterm(); });
    }
    ...
    ProcessInformation getProcessesInformation() {
        ProcessInformation result = ProcessInformation(ProcessInfoSource.APPLICATION, _("Aiterm"), "", []);
        foreach(window; appWindows) {
...
//            acl.destroy();
    ...
    void addOptions() {
        ...
        if (Version.checkVersion(3, 16, 0).length ==0) {
            addMainOption(CMD_ACTION, 'a', GOptionFlags.NONE, GOptionArg.STRING, _("Send an action to current Aiterm instance"), _("ACTION_NAME"));
        }
...
public:
    ...
    this(bool newProcess, string group=null) {
        ...
        if (group.length > 0) {
            string id = "com.aiterm.Aiterm." ~ group;
            if (idIsValid(id)) {
    ...
    */
     ...
     * through Aiterm, see #666 fore more information.
     */
    override void setAccelsForAction(string detailedActionName, string[] accels) {
        import gx.aiterm.terminal.actions;

        if (detailedActionName == getActionDetailedName(gx.aiterm.terminal.actions.ACTION_PREFIX, gx.aiterm.terminal.actions.ACTION_PASTE)) {
            accels ~= ["<Shift><Ctrl>Insert"];
        } else if (detailedActionName == getActionDetailedName(gx.aiterm.terminal.actions.ACTION_PREFIX, gx.aiterm.terminal.actions.ACTION_PASTE_PRIMARY)) {
            accels ~= ["<Shift>Insert"];
        }
    ...
    /** VTE shell-integration warning disabled until user-facing docs exist. */
