 ...
 */
module gx.aiterm.terminal.exvte;
...
static if (COMPILE_VTE_BACKGROUND_COLOR) {
    ...
    /** Paste arbitrary text (gtk-d 3.10 bindings omit this VTE API). */
    void pasteText(string text) {
        if (text.length == 0) return;
        vte_terminal_paste_text(vteTerminal, text.ptr, cast(ssize_t)text.length);
    }
}
...
__gshared extern(C) {
	...
	void function(VteTerminal* terminal, const(char)* text, ssize_t length) c_vte_terminal_paste_text;
