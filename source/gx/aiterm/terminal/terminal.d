 ...
 */
module gx.aiterm.terminal.terminal;
...
import gx.aiterm.terminal.actions;
...
import gx.aiterm.terminal.monitor;
import gx.aiterm.terminal.activeprocess;
...
private:
    ...
    Widget createTitlePane() {
        ...
        lblTitle.getStyleContext().addClass("aiterm-terminal-title");
        ...
        spBell.getStyleContext().addClass("aiterm-bell");
    ...
    void createActions(SimpleActionGroup group) {
        ...
        registerActionWithSettings(group, ACTION_PREFIX, ACTION_PROFILE_PREFERENCE, gsShortcuts, delegate(GVariant, SimpleAction) {
            aiterm.presentProfilePreferences(prfMgr.getProfile(_activeProfileUUID));
        }, null, null);
        ...
        registerAction(group, ACTION_PREFIX, ACTION_ENCODING_PREFERENCE, null, delegate(GVariant, SimpleAction) {
            aiterm.presentEncodingPreferences();
        }, null, null);
    ...
    GMenuItem createAddButtons() {
        ...
        addH.setAttributeValue("verb-icon", new GVariant("aiterm-add-horizontal-symbolic"));
        addH.setAttributeValue("label", new GVariant(_("Add Right")));

        GMenuItem addV = new GMenuItem(null, "session.add-down");
        addV.setAttributeValue("verb-icon", new GVariant("aiterm-add-vertical-symbolic"));
    ...
    Widget createVTE() {
        ...
        vteHandlers ~= vte.addOnContentsChanged(delegate(VTE) {
            ...
            if (terminalInitialized && aiterm.testVTEConfig() && gst.currentLocalDirectory.length == 0 && _overrideCommand.length == 0) {
        ...
        vteHandlers ~= vte.addOnKeyRelease(delegate(Event event, Widget widget) {
            ...
            if ((keyval == GdkKeysyms.GDK_c) && (event.key.state & ModifierType.CONTROL_MASK)) {
                string[] actions = aiterm.getActionsForAccel("<Ctrl>c");
                if (actions.length > 0 &&
        ...
        if (useOverlayScrollbar) {
            ...
            sw.getStyleContext.addClass("aiterm-terminal-scrolledwindow");
        ...
        if (!useOverlayScrollbar) {
            ...
            sb.getStyleContext().addClass("aiterm-terminal-scrollbar");
    ...
    void processTrigger(TerminalTrigger trigger, string[] groups) {
        ...
        final switch (trigger.action) {
            ...
            case TriggerAction.SEND_NOTIFICATION:
                ...
                string title = _("Aiterm Custom Notification");
                string _body;
    ...
    void applyPreference(string key) {
        ...
        SETTINGS_PROFILE_BG_TRANSPARENCY_KEY, SETTINGS_PROFILE_DIM_TRANSPARENCY_KEY:
            ...
            if (!useOverlayScrollbar) {
                ...
                sbProvider = createCssProvider(APPLICATION_RESOURCE_ROOT ~ "/css/aiterm." ~ theme ~ ".scrollbar.css", variables);
                if (sbProvider !is null) {
...
private:
    ...
    void spawnTerminalProcess(string workingDir, string command = null) {
        ...
        CommandParameters overrides = aiterm.getGlobalOverrides();
    ...
    /*
     * A thin wrapper over sendHostCommand that asks the aiterm-flatpak-toolbox for information
    ...
    string captureHostToolboxCommand(string command, string arg, int[] extra_fds) {
        ...
        string[] args = [format("%s/bin/aiterm-flatpak-toolbox", hostRoot), command, arg];
    ...
    void setProxyEnv(ref string[] envv) {
        ...
        GSettings gsProxy = aiterm.getProxySettings();
...
public:
    ...
    this(string profileUUID, string requestedUUID) {
        ...
        if (aiterm.getGlobalOverrides().profileName.length > 0) {
            string newProfileUUID = prfMgr.getProfileUUIDFromName(aiterm.getGlobalOverrides().profileName);
        ...
        if (aiterm.getGlobalOverrides().title.length > 0) {
            _overrideTitle = aiterm.getGlobalOverrides().title;
        }
        ...
        aiterm.onThemeChange.connect(&onThemeChanged);
    ...
    void initTerminal(string initialPath, bool firstRun) {
        ...
        if (firstRun) {
            ...
            if (aiterm.getGlobalOverrides.geometry.flag != GeometryFlag.NONE) {
                width = aiterm.getGlobalOverrides().geometry.width;
                height = aiterm.getGlobalOverrides().geometry.height;
            }
    ...
    void finalizeTerminal() {
        ...
        stopProcess();
        aiterm.onThemeChange.disconnect(&onThemeChanged);
