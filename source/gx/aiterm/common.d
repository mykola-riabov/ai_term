...
/**************************************************************************
 * Defines interfaces that aiterm uses to reference objects without creating
...
interface ITerminal : IIdentifiable {
    ...
    /**
     * Injects text into the shell (as if typed), with trailing newline if missing.
     */
    void injectCommand(string text);

    /** Send SIGINT to shell (Ctrl+C). */
    void sendInterrupt();
