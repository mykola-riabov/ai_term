...
public:
    this() {
        ...
        lbSessions.getStyleContext().addClass("aiterm-session-sidebar");
...
private:
    ...
    void createUI(Session session, SessionNotification[string] notifications, int width, int height) {
        ...
        afNotification = wrapWidget(evNotification, "aiterm-notification-count");
        ...
        lblName.getStyleContext().addClass("aiterm-session-name");
        ...
        grid.attach(wrapWidget(lblIndex, "aiterm-session-index"), 2, 2, 1, 1);

        //Add Close Button
        btnClose = new Button("window-close-symbolic", IconSize.MENU);
        btnClose.getStyleContext().addClass("aiterm-sidebar-close-button");
