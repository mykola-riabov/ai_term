 ...
 */
module gx.aiterm.prefeditor.prefdialog;
...
import gx.aiterm.modern.prefpage;
...
private:
    ...
    void createSplitHeaders() {
        ...
        sTitle.getStyleContext().addClass("aiterm-title-separator");
        bTitle.add(sTitle);
        bTitle.add(hbMain);
    ...
    void updateShortcutSetting(TreeIter iter, string shortcut) {
        if (tsShortcuts.getValueString(iter, COLUMN_SHORTCUT_TYPE) == SC_TYPE_ACTION) {
            ...
            if (shortcut != SHORTCUT_DISABLED) {
                aiterm.addAccelerator(shortcut, getActionDetailedName(ACTION_PREFIX_TERMINAL,ACTION_PROFILE_SELECT), new GVariant(uuid));
            } else {
                aiterm.removeAccelerator(getActionDetailedName(ACTION_PREFIX_TERMINAL,ACTION_PROFILE_SELECT), new GVariant(uuid));
            }
        }
    }
