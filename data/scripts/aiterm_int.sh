...
__aiterm_osc7() (
  ...
  printf "\033]7;file://%s%s\007" "${HOSTNAME:-}" "$(__aiterm_urlencode "${PWD}")"
  return $previous_exit_status
)

if [[ $PROMPT_COMMAND != *"__vte_prompt_command"* ]]
then
    if [ "$AITERM_SILENT" != "1" ]; then
        echo "Adding aiterm osc7"
    fi
    [ -n "$BASH_VERSION" ] && PROMPT_COMMAND="__aiterm_osc7"
    [ -n "$ZSH_VERSION"  ] && precmd_functions+=(__aiterm_osc7)
fi
