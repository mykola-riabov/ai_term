...
TERMINAL = shutil.which("aiterm")
TILIX_KEYBINDINGS = "com.aiterm.Aiterm.Keybindings"
...
if hasattr(Nautilus, "LocationWidgetProvider"):
    class OpenAitermShortcutProvider(GObject.GObject,
...
class OpenAitermExtension(GObject.GObject, Nautilus.MenuProvider):
        ...
        if file_.is_directory():
            ...
            if file_.get_uri_scheme() in REMOTE_URI_SCHEME:
                ...
                item = Nautilus.MenuItem(name='NautilusPython::open_remote_item',
                                         label=_(u'Open Remote Aiterm'),
                                         tip=_(u'Open Remote Aiterm In {}').format(uri))
            ...
            item = Nautilus.MenuItem(name='NautilusPython::open_file_item',
                                     label=_(u'Open In Aiterm'),
                                     tip=_(u'Open Aiterm In {}').format(filename))
            item.connect('activate', self._menu_activate_cb, file_)
            items.append(item)
    ...
    def get_background_items(self, *args):
        ...
        if file_.get_uri_scheme() in REMOTE_URI_SCHEME:
            item = Nautilus.MenuItem(name='NautilusPython::open_bg_remote_item',
                                     label=_(u'Open Remote Aiterm Here'),
                                     tip=_(u'Open Remote Aiterm In This Directory'))
            item.connect('activate', self._menu_activate_cb, file_)
            items.append(item)

        item = Nautilus.MenuItem(name='NautilusPython::open_bg_file_item',
                                 label=_(u'Open Aiterm Here'),
                                 tip=_(u'Open Aiterm In This Directory'))
